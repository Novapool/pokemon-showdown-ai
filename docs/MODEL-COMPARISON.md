...
| MCTS over M8 Phase 2 value-head fine-tune, targets collected **vs DamageFirst** (`--target outcome`) | inference-time value-head-only FT | — | **80.0% (160/200)** vs base **82.5% (165/200)** | ❌ **Negative result, replication (M8 Phase 2 retry, 2026-07-30).** Same recipe as the row above but the 2000 games of targets were collected against DamageFirst — the bot the A/B is scored on — to test the distribution-mismatch explanation. **Landed on exactly the same 160/200: −2.5pp, Criterion C failed again.** In-distribution the fine-tune again worked (57,747 decisions; val MSE 0.6917 → 0.4915 vs a constant-predictor baseline of 0.610, R² **−0.13 → +0.19**; sign agreement 0.758 → 0.840). **Distribution mismatch is ruled out as the explanation.** Caveat that cuts the other way: labels are far more skewed here (target mean **+0.624**, searcher won ~84%), and sign agreement 0.840 ≈ the base rate — consistent with the surviving explanation that the MSE gain is mostly learning "usually wins", a constant offset MCTS's *relative* leaf comparison ignores. Each delta is inside noise (SE ~3.9pp at n=200); the finding is two independent nulls, not one significant one. Logs: `models/mcts/results/m8_phase2_df_valft_{criterionC,train}.log`; checkpoint `models/ppo/checkpoints/v3_valft/ppo_v3_df_valft_outcome.pt` (home box only — not committed) |
| **MCTS over M7 obs-v3 final (policy sampler, tuned knobs)** | inference-time | **93.0% (465/500)** | **84.2% (421/500)** | ✅ **Current best agent by bot eval** (+2.4pp R / +5.0pp DF over the M5.5 best). **Ladder read (Criterion C, 100 games): Elo 1034.6 / GXE 28.2%, vs M6's 1017/23.9%** — directionally up (+9pp raw win rate) but lands in the pre-registered 25–34% inconclusive band, not a confirmed ladder win. ⚠️ **Three later readings on this same checkpoint were 42% raw (n=50, 7/23), then 27% raw (n=100, 7/31, M8 Phase 4, Elo 1084.0 / GXE 32.9%) — a 15pp swing at zero true effect.** The "monotonic 23.9 → 28.2 → 32.9 GXE trend" once recorded here was ladder drift, not progress; GXE is an account-level cumulative statistic over 506 games and was never a valid per-run gate. **Do not quote any ladder figure in this table without reading `docs/LADDER-MEASUREMENT.md` first.** Eval logs: `models/mcts/results/m7_v3_mcts_{random,damagefirst}.log`; ladder log: `data/replays/self_ladder/m7_ladder_run.log` |
