...
**This file is read on demand, not at session start.** For the live plan and a
condensed results ledger, see `MILESTONES.md`. For the one-screen orientation,
see `docs/WHERE-WE-ARE.md`.

Two reading notes:

- **Plans here are historical, not current.** "Files to Create", "Hyperparameters
  (starting point)", and "Build Phasing" sections describe what was intended
  before the work ran. Where the build deviated, the "What Was Built" sections
  are authoritative — and the code is more authoritative still.
- **Corrections are inline.** Blocks marked `⚠️ CORRECTION` were added after the
  fact (mostly 2026-08-01, from `docs/CODE-REVIEW-FINDINGS.md`) and override the
  surrounding text. The two that matter most: the MCTS-vs-raw comparisons are
  confounded by a greedy-vs-sampling decision-rule swap (§3), and the "~3pp seat
  bias" is a convention that was never measured (§5d).
...
## M8: Value-Head Targeting + Ladder Infrastructure ✅ COMPLETE — ALL BETS NEGATIVE OR INCONCLUSIVE

**Status:** ✅ Complete 2026-07-31. Both technical bets spent, neither paid;
the durable finding is methodological (GXE is account-level and was never a
valid per-run gate). Condensed verdict in `MILESTONES.md`; per-phase results
are recorded inline against each criterion below.

**Originally scoped:** 2026-07-22 (the plan text that follows predates
execution).
...
## M9: Evaluation Methodology + Data Distribution Hypothesis ✅ COMPLETE

**Status:** ✅ Complete 2026-08-01. All three phases executed; two hypotheses
closed, one strongly suggestive. Condensed verdict in `MILESTONES.md`;
per-phase results are recorded inline below.

**Originally scoped:** 2026-07-31 (the plan text that follows predates
execution).
...
# Appendix: Execution Notes (moved from IN-PROGRESS.md, 2026-08-03)

Run-level records that were accumulating in `IN-PROGRESS.md` long after
their milestones closed. Kept because they contain measured numbers and
runbook detail that the milestone summaries above compress away.

---

## M9 execution detail (phases, gates, and the seed replication)
...
## M5 Phase 3 smoke battery (2026-07-16)
...
## M7 job logs (Criterion A, BC pretrain, PPO fine-tune)
...
# M10–M12 (closed; moved here from MILESTONES.md 2026-09-24)

Full text of the last three research milestones, verbatim as they stood at
close. M10 and M11 were closed without execution on 2026-08-05; M12 closed
2026-09-24 with Phase 5's ladder result unrecovered (home box offline). The
condensed conclusions live in `MILESTONES.md` → Results Ledger.
