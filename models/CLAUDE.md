...
| `tracking.py` | M13: MLflow run tracking for `ppo/train.py`, `bc_pretrain_mlp.py` and `evaluate.py`. Logs args, git SHA/dirty, seed and checkpoint path + sha256 to `mlflow.db` (repo root, Mac-local). Adds `--seed`/`--no-track` to each. No-op without mlflow; fails soft if the store drops. `evaluate.py --json-out` feeds the CI gate (`.github/workflows/eval-smoke.yml`; threshold in `docs/EVALUATION-METHODOLOGY.md` Part 6). List/check runs: `scripts/mlflow_runs.py` |
...
**Decision rule — `--greedy` (2026-08-01):** `PPOAgent`/`TransformerAgent` `act()`/`act_batch()` **sample** from the masked policy by default; setting `agent.greedy = True` switches them to the masked argmax. Exposed as `--greedy` on `evaluate.py` and `infer_server.py`. **`ladder-bot.js` is greedy by DEFAULT as of 2026-08-01** — `--sample` opts back out (that is the control arm of any greedy A/B, and the pre-2026-08-01 behaviour); its startup banner prints `policy=greedy|sampled`, so no session is ambiguous about its decision rule. Under `--mcts` the search already argmaxes, so the decision rule only reaches raw-policy fallbacks (force switches / locked states, `ladder-bot.js:291`) — an `--mcts` greedy A/B tests a fraction of the change. Rollouts must sample, so the default is False and the trainer never touches it. `evaluate.py --greedy` rejects `--model mcts` (search already argmaxes over root visit counts) and q_learning/dqn (already ε=0), and prints the decision rule in its results header; with `--vs-checkpoint` only the `--checkpoint` arm goes greedy, so an h2h run *is* the greedy-vs-sampled A/B. ⚠️ **Every number in this project dated before 2026-08-01 — including the 93.0% MCTS figure, the 69.7% raw figure and the 30.5% ladder result — was produced on the sampling path**, and every search-vs-no-search comparison silently varied the decision rule alongside search (`docs/CODE-REVIEW-FINDINGS.md` §3). Measured on the M7 v3 checkpoint, n=5,000/arm, one session: **+7.8pp vs Random [+6.1, +9.5]** (69.9% → 77.7%) and **+5.0pp vs DamageFirst [+3.1, +6.9]** (60.2% → 65.2%). Both gates pass; the second *reverses* the −2.0pp the review read at n=400. Greedy on the **ladder** is still untested — determinism against adapting humans is the one thing bot evals cannot speak to.

**Fixed roster (M12, 2026-08-05):** `--format gen1ou` on `evaluate.py`, `ppo/train.py` and `gym_bridge.js` (and `battle_format=`/`roster=` on `GymClient`/`VecGymClient`) switches to fixed-team Gen 1 OU and hands the **same** pre-registered packed team to **both** seats, removing team luck. The roster lives in `config/rosters/gen1ou-standard.txt` and is loaded by `sim/tools/roster.ts`, which **throws** if the file is missing rather than silently falling back to a generated team. `--roster <file>` overrides. No `--format` ⇒ gen1randombattle, unchanged. `evaluate.py` prints the format and `FIXED ROSTER (both sides)` in its results header. **MCTS determinization is roster-aware**: under a mirror roster the opponent's bench is known, so `BattleSim` fills it from the roster instead of sampling — without this, `Teams.getGenerator('gen1ou')` silently falls back to the gen1 *random* generator and search models the opponent as Pokemon they cannot have. The ladder path (`BattleSim.fromTracked`) still samples deliberately, because ladder opponents bring their own teams. **BC needs no changes** — the v3 observation carries no species or stats, so the roster is invisible to the encoder. Roster rationale, caveats (~6% draws, ~111-turn games, no seat bias at n=600) and runnable commands: `docs/BATTLE-FORMATS.md`.

**Head-to-head (M3.3):** `evaluate.py --vs-checkpoint <path>` plays the `--checkpoint` agent (seat p1) directly against a second PPO checkpoint (seat p2) through the dual-seat bridge. PPO-only; both checkpoints must share the obs mode (`--structured` applies to both); mutually exclusive with `--opponent`. **Run both orientations and combine** — `_run_battles_h2h` hard-seats the `--checkpoint` agent as p1 and there is no `--seat` flag, so a single-orientation h2h is unbalanced by construction and nothing in the code enforces the correction. ⚠️ **The "~3pp seat bias" this note used to assert as fact is not supported by its own measurement** (2026-08-01 review): the only direct reading is `docs/MILESTONES-ARCHIVE.md:588` (M3.4 head-to-head), 45% vs 51% at n=500, whose CI on the difference *includes 0*, and M4's 60.4%/60.0% shows no effect either. An unmeasured constant was serving as an error bar. Seat bias here is **unmeasured, not known to be ~3pp** — balance orientations because it is free, not because a 3pp correction has been established, and do not quote 3pp as a tolerance. See `docs/CODE-REVIEW-FINDINGS.md` §5d.
